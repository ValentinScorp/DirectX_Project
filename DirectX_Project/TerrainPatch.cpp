...
TerrainPatch::TerrainPatch(int x, int y, int tilesDim, float tileSize)
{
	...
	for (int i = 0; i < tilesDim; i++) {
		for (int j = 0; j < tilesDim; j++) {
			...
			D3DXVECTOR3	tp2(xPos + j * tileSize, yPos + i * (tileSize + 1), 0.0f);
			D3DXVECTOR3	tp3(xPos + j * (tileSize + 1), yPos + i * (tileSize + 1), 0.0f);
			D3DXVECTOR3	tp4(xPos + j * (tileSize + 1), yPos + i * tileSize, 0.0f);
